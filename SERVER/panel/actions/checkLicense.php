...
session_start();
if(empty($_SESSION["pqcms-server-admin-logged"])) die("You must be logged in.");
if(empty($_POST["server"]) || empty($_POST["domain"]) || empty($_POST["login"]) || empty($_POST["license_key"])) die("Got wrong config. Check your posts.");

if(!isset($_SESSION["pqcms-server-token-checklicense"]) || !isset($_SESSION["pqcms-server-token-checklicense-expire"]))
    die("Wrong token. Reload the form.");
if(time() >= $_SESSION["pqcms-server-token-checklicense-expire"])
    die("Token has expired. Reload the form.");
if($_SESSION["pqcms-server-token-checklicense"] != $_POST["token"])
    die("Incorrect token.");
